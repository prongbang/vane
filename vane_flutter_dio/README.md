...
- Response headers are single-valued (repeated ones already comma-joined by the
  core), so each becomes a one-element list. `set-cookie` is not surfaced at
  all: use Vane's own cookie jar.
